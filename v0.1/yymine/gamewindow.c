...
extern const TCHAR szCubicClass[];
extern const TCHAR szFaceClass[];
extern const TCHAR szCounterScreenClass[];
extern const TCHAR szTimeScreenClass[];
...
void GameWindowRegisterCubic(const TCHAR szclsName[])
{
    ...
    {
        MessageBox(NULL, TEXT("cubic class register error"), szclsName, MB_ICONERROR);
        exit(EXIT_FAILURE);
    }
...
void GameWindowRegisterFace(const TCHAR szclsName[])
{
    ...
    wndclass.style = CS_HREDRAW | CS_VREDRAW;
    wndclass.lpfnWndProc = FaceWndProc;
    wndclass.cbClsExtra = 0;
    wndclass.cbWndExtra = 0;
    wndclass.hInstance = hInst;
    wndclass.hIcon = 0;
    ...
    if (!RegisterClass(&wndclass))
    {
        MessageBox(NULL, TEXT("face class register error"), szclsName, MB_ICONERROR);
        exit(EXIT_FAILURE);
    }
...
void GameWindowRegisterTimeScreen(const TCHAR szclsName[])
{
    ...
    if (!RegisterClass(&wndclass))
    {
        MessageBox(NULL, TEXT("screen class register error"), szclsName, MB_ICONERROR);
        exit(EXIT_FAILURE);
    }
...
void GameWindowRegisterCounterScreen(const TCHAR szclsName[])
{
    ...
    wndclass.style = CS_HREDRAW | CS_VREDRAW;
    wndclass.lpfnWndProc = CounterScreenWndProc;
    wndclass.cbClsExtra = 0;
    wndclass.cbWndExtra = 0;
    wndclass.hInstance = hInst;
    wndclass.hIcon = 0;
...
POINT GameWindowGetSize(void)
{
    ...
    size.x = 12 + 16 * pgs->icCol + 8 +
        GetSystemMetrics(SM_CXSIZEFRAME) - 2 + 13; //width
    size.y = 9 + 37 + 9 + 16 * pgs->icRow + 8 +
        GetSystemMetrics(SM_CYCAPTION) +
        GetSystemMetrics(SM_CYMENU) + 5 + 9; //height
    ReleasePGS(pgs);
...
{
    PGAMESTRUCT pgs = GetPGS();
    pgs->hwndFace = CreateWindow(szFaceClass, NULL,
                                 WS_CHILDWINDOW | WS_VISIBLE,
                                 startPt.x, startPt.y,
                                 24, 24,
...
{
    PGAMESTRUCT pgs = GetPGS();
    pgs->hwndTimeScreen = CreateWindow(szTimeScreenClass, NULL,
                                       WS_CHILDWINDOW | WS_VISIBLE,
                                       startPt.x, startPt.y,
                                       39, 23,
...
{
    PGAMESTRUCT pgs = GetPGS();
    pgs->hwndCounterScreen = CreateWindow(szCounterScreenClass, NULL,
                                          WS_CHILDWINDOW | WS_VISIBLE,
                                          startPt.x, startPt.y,
                                          39, 23,
...
HWND GameWindowCreateFrame(void)
{
    ...
    hwnd = CreateWindow(szAppname, TEXT("mine creeper"),
